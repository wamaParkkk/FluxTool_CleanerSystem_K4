...
namespace FluxTool_CleanerSystem_K4
{
    public partial class PM3Form : UserControl
    {
        ...
        public void Display()
        {
            ...
            // Output display           
            if ((Global.digSet.curDigSet[(int)DigOutputList.CH3_Cylinder_Pwr_o] == "On") &&
                (Global.digSet.curDigSet[(int)DigOutputList.CH3_Cylinder_FwdBwd_o] == "Off"))
            {
                textBoxCylinderFwdBwd.Text = "Forward";
            }
            else if ((Global.digSet.curDigSet[(int)DigOutputList.CH3_Cylinder_Pwr_o] == "On") &&
                     (Global.digSet.curDigSet[(int)DigOutputList.CH3_Cylinder_FwdBwd_o] == "On"))
            {
        ...
        private async void Digital_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            switch (strTmp)
            {
                ...
                case "2":
                    {
                        digitalDlg.Init("Close", "Open", "CH3 Curtain Air Valve");
                        if (digitalDlg.ShowDialog() == DialogResult.OK)
                        {
                            if (digitalDlg.m_strResult == "Close")
                            {
